...
for (let i = 0; i < galaxyParameters.count; i++) {
  ...
  const randomX = (Math.random() - 0.5) * galaxyParameters.randomness * radius * 2;
...
for (let group = 0; group < numGroups; group++) {
  ...
  img.onload = () => {
    const neonTexture = createNeonTexture(img, 1000);
    ...
    // Material khi ở gần
      ...
      vertexColors: true,
      emissive: new THREE.Color(0xffffff), // Đảm bảo không bị ảnh hưởng bởi ánh sáng
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    ringGroup.userData = {
      ...
      speed: 0.0012 + 0.000025, // Tốc độ quay
...
function preloadGalaxyAudio() {
  const audioSources = [
   "https://cdn.discordapp.com/attachments/1379317837503402005/1402701613860126941/sinh_nhat.MP3?ex=6896d96d&is=689587ed&hm=ea204f2842593b28ff994d85a98a89415ec48d087f973c7938d1c632737df1e5&",
  ];
...
function createStarFrameTexture(size = 128) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  ...
  for (let i=0; i<5; i++) {
    ctx.lineTo(
      cx + r * Math.cos((18 + 72 * i) * Math.PI / 180),
      cy - r * Math.sin((18 + 72 * i) * Math.PI / 180)
    );
    ctx.lineTo(
      cx + (r/2) * Math.cos((54 + 72 * i) * Math.PI / 180),
      cy - (r/2) * Math.sin((54 + 72 * i) * Math.PI / 180)
    );
  }
  ctx.closePath();
  ctx.lineWidth = 6;
...
function createHeartFrameTexture(size = 128) {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext('2d');
  const cx = size / 2, cy = size / 2;
  const scale = size * 0.010;
...
function addRandomFrames(type = 'star', count = 5) {
  for(let i=0; i<count; i++) {
    let texture;
    if(type === 'star') texture = createStarFrameTexture();
    else texture = createHeartFrameTexture();
    
    const material = new THREE.SpriteMaterial({ map: texture, transparent: true });
    const sprite = new THREE.Sprite(material);
    sprite.scale.set(13, 13, 1);
    // Vị trí ngẫu nhiên trong không gian quanh gốc
    sprite.position.set(
      (Math.random() - 0.5) * 180,
...
// Gọi hàm để thêm 5 ngôi sao và 5 trái tim
addRandomFrames('star', 20);
addRandomFrames('heart', 20);
